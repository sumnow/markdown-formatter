<!--
Created: Mon Nov 21 2022 17:22:26 GMT+0800 (China Standard Time)
Modified: Fri Jun 09 2023 22:40:08 GMT+0800 (China Standard Time)
-->

# Log

[中文文档](./CHANGELOG_CN.md)

## 0.9.9

* add feature: table alignment

## 0.9.8

* Add unit tests
* Add tslint 

## 0.9.7

* Fix issue: code block appear error sometimes

## 0.9.6

* Update snippets

## 0.9.5

* Update `CHANGELOG.md`

## 0.9.4

* New Feature: Disable format file tag is optional
* Fix issue: [How to disable formatter with a file](hhttps://github.com/sumnow/markdown-formatter/issues/41)
* Update `CHANGELOG.md`

## 0.9.3

* Fix issue: [yaml code blocks get formatted ](https://github.com/sumnow/markdown-formatter/issues/49)
* Update `CHANGELOG.md`

## 0.9.2

* Fix issue: [Autoformat incorrectly adding spaces in code fence](https://github.com/sumnow/markdown-formatter/issues/35)
* Update `CHANGELOG.md`

## 0.9.1

* Fix issue: [格式化序号有bug #45](https://github.com/sumnow/markdown-formatter/issues/45)
* Fix issue: [Spaces being added forever between comma and snippet](https://github.com/sumnow/markdown-formatter/issues/48)
* Update `CHANGELOG.md`

## 0.9.0

* Update `CHANGELOG.md`

## 0.8.9

* Fix issue: [Formatting issue in shell/bash code block](https://github.com/sumnow/markdown-formatter/issues/38)
* Fix issue: [Space remove in case of list entry ending with code](https://github.com/sumnow/markdown-formatter/issues/36)
* Update `CHANGELOG.md`

## 0.8.8

* Fix issue: [错误地加转义符号](https://github.com/sumnow/markdown-formatter/issues/29)
* Update `CHANGELOG.md`

## 0.8.7

* Fix issue: [Formatter adds trailing space between backticks and newlines, which conflicts with Markdown linters](https://github.com/sumnow/markdown-formatter/issues/31)
* Update `CHANGELOG.md`

## 0.8.6

* Fix issue: There is no content between consecutive titles, and blank lines cannot be formatted correctly.
* Update `CHANGELOG.md`

## 0.8.5

* Fix bug: [[Error Format]: TypeError: Cannot read property 'forEach' of null](https://github.com/sumnow/markdown-formatter/issues/28)
* Update `CHANGELOG.md`

## 0.8.4

* Fix bug: [引用下的表格](https://github.com/sumnow/markdown-formatter/issues/20)
* Update `CHANGELOG.md`

## 0.8.3

* Remove notification
* add configuration `spaceAfterFullWidthOrHalfWidth`

* Update `CHANGELOG.md`

## 0.8.2

* Fix bug
* Update `CHANGELOG.md`

## 0.8.1

* Fix bug: [2 blank lines added at the end of file, conflicts with markdownlint](https://github.com/sumnow/markdown-formatter/issues/22)
* Fix bug: [能否不要删除code block](https://github.com/sumnow/markdown-formatter/issues/18)

## 0.8.0

* Fix bug: [It's not formatting properly lines that contain regexp](https://github.com/sumnow/markdown-formatter/issues/22)
* Fix bug: [Formatting adds a "*" to Markdown Rules](https://github.com/sumnow/markdown-formatter/issues/23)

## 0.7.9

* Fix bug: [列表接着一级标题格式化出错 #16](https://github.com/sumnow/markdown-formatter/issues/16)
* Update `CHANGELOG.md`

## 0.7.8

* Update `README.md`
* Update `CHANGELOG.md`

## 0.7.7

* Upload again

## 0.7.6

* Fix bug: [bad formatting for dots](https://github.com/sumnow/markdown-formatter/issues/13) 
* Update `CHANGELOG.md`

## 0.7.5

* Fix bug: [格式化错误](https://github.com/sumnow/markdown-formatter/issues/12) 
* Fix bug: The `>` formatted as link in code block cause error.
* Update `CHANGELOG.md`

## 0.7.4

* Fix bug: the code area will formatted failed sometimes.
* Update snippets discribution
* Update `CHANGELOG.md`

## 0.7.3

* Fix bug: the list formatted failed sometimes.
* Update `CHANGELOG.md`

## 0.7.2

* Fix bug: the code after list will formatted failed
* Update `CHANGELOG.md`

## 0.7.1

* Fix bug
* Update `CHANGELOG.md`

## 0.7.0

* Fix bug: the title will not after breakline, while `displayTime` is `true` .
* Update `CHANGELOG.md`

## 0.6.9

* Fix bug: the code wrapped by `*` , `~~` , `**` or `***` will not formatted as expected.
* Update `CHANGELOG.md`

## 0.6.8

* Update `README.md`

## 0.6.7

* Cut multiple spaces after the symbol to 1
* Add the feature of replacing the full corner with a comma
* Update `README.md`
* Update `CHANGELOG.md`

## 0.6.6

* New feature: display time before the article

## 0.6.5

* All blank lines before H1 had been removed
* Update `CHANGELOG.md`

## 0.6.4

* A blank line in the code block is treated as continuous
* Update `README.md`

* Update `CHANGELOG.md`

## 0.6.3

* Add new `Snippets` : 'time'  and optimizate others
* Update `CHANGELOG.md`

## 0.6.2

* Back quote after breakline will not after a space now.
* Update `CHANGELOG.md`

## 0.6.1

* Code area will not formatted if `codeAreaToBlock` as default 
* Update `README.md`

* Update `CHANGELOG.md`

## 0.6.0

* Remove property `codeAreaFormat`

* Update `README.md`

* Update `CHANGELOG.md`

## 0.5.9

* Fix bug: Code area will not formatted as expected.
* Optimization
* Update `CHANGELOG.md`

## 0.5.8

* Optimization
* Update `CHANGELOG.md`

## 0.5.7

* Fix bug: Convert code block to code area will failed sometimes
* Update `CHANGELOG.md`

## 0.5.6

* New feature: Serial number alignment in ordered list
* Update `CHANGELOG.md`

## 0.5.5

* Fix bug: [半角符号不能转换](https://github.com/sumnow/markdown-formatter/issues/5)
* Fix bug: [格式化操作会破坏 `markdown-all-in-one` 插件生成的目录](https://github.com/sumnow/markdown-formatter/issues/8)
* Fix bug: [格式化操作会破坏原有链接](https://github.com/sumnow/markdown-formatter/issues/9)
* Update `CHANGELOG.md`

## 0.5.4

* Fix bug: [关于嵌套的连接的格式化问题](https://github.com/sumnow/markdown-formatter/issues/7)
* Fix bug: [对于引用区域的格式化是否可以控制不要添加多余的行?](https://github.com/sumnow/markdown-formatter/issues/6)
* Update `CHANGELOG.md`

## 0.5.3

* Fix bug: Format HTML and CSS in code block
* Update `CHANGELOG.md`

## 0.5.2

* Fix bug: Code area will not formatted as expected.
* Update `CHANGELOG.md`

## 0.5.1

* New feature: Convert full-width symbols to half-width symbols by config `fullWidthTurnHalfWidth`

* Update `README.md`

* Update `CHANGELOG.md`

## 0.5.0

* Fix bug: code area next to each other will be formatted error
* Update `CHANGELOG.md`

## 0.4.9

* New feature: Convert code block to code area 
* Update `README.md`

* Update `CHANGELOG.md`

## 0.4.8

* Format code block's languages' types 
* Update `README.md`

* Update `CHANGELOG.md`

## 0.4.7

* Update `README.md`

* Update `CHANGELOG.md`

## 0.4.6

* Fix bug: Code area in the end will not be formatted as expected
* Update `CHANGELOG.md`

## 0.4.5

* Formatted uneded list symbol, as * > + > -
* Update `CHANGELOG.md`

## 0.4.4

* Fix Bug: the third level list is not aligned
* Update `CHANGELOG.md`

## 0.4.3

-The spaces after the full-width symbol are optional now! 
[搞不懂为什么要把全角符号改成半角的](https://github.com/sumnow/markdown-formatter/issues/3)
* Update `CHANGELOG.md`

## 0.4.2

* Snippets supports more languages, such as `java` , `python` , etc.
* Fix formatting errors in multiple code areas
* Update `README.md`

* Update `CHANGELOG.md`

## 0.4.1

* Update `README.md`

* Update `CHANGELOG.md`

## 0.4.0

* Add feature: Snippets
* Update `README.md`

* Update `CHANGELOG.md`

## 0.3.4

* Empty a line for table; 
* Empty a line for image tag; 
* Update `README.md`

* Update `CHANGELOG.md`

## 0.3.3

* Update `README.md`

## 0.3.2

* Optimize code structure
* Update `README.md`

* Provides the ability to format `CSS` and `HTML`

* Fix bug

## 0.3.1

* Optimize code structure
* Update `README.md`

* Update `CHANGELOG.md`

## 0.3.0

* Fixbug: [error in code](https://github.com/sumnow/markdown-formatter/issues/1)

## 0.2.9

* Update `CHANGLOG.md`

## 0.2.8

* Update images in `README.md`

## 0.2.7

* Update `README.md`

* Supports converting Chinese symbols into full-width symbols according to context, or converting English into half-width symbols; 

## 0.2.6

* Update `README.md`

* Format `?!.` correctly

## 0.2.5

* Update `README.md`

## 0.2.4

* Fix bug: sometime format failed.

## 0.2.3

* Update `README.md`

## 0.2.2

* Fix bug

## 0.2.1

* Avoid duplicate formatting

## 0.2.0

* Update `README.md`

## 0.1.9

* Format period correctly now!

## 0.1.8

* Fix bug: chinese in table be formatted

## 0.1.7

* Fix bug: break line when list's length more than 9

## 0.1.6

* Fix list problem in code area

## 0.1.5

* Improve js-beautify's content

## 0.1.4

* Fix list bug

## 0.1.3

* Add charctar '? ' to '?'

## 0.1.2

* Fix bug: code area next code block not formatted as expected.

## 0.1.1

* Add feature: list formatted
* Add feature: fullwidth to halfwidth

## 0.1.0

* Fix bug: format table  failed

## 0.0.9

* Fix bug: code area formatted failed

## 0.0.8

* Enable/disable format code 

## 0.0.7

* Fix bug: code dulicated formatted
* Fix bug: PC / UNIX endofline
* Update README.md

## 0.0.6 

* Fix link block bug
* Update READEME.md and icon

## 0.0.5

* Fix bug 

## 0.0.4

* Erase dulicated line

## 0.0.3

* Empty a line for the title; 

## 0.0.2

* Fix the back-quote problem in the code block; 

## 0.0.1  

* Initial release of the tidy-markdown extension; 
